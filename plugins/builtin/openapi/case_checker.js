...
const dashCaseRegex = /^[a-z]+(-[a-z]+)*$/;

// Kudos:https://github.com/sindresorhus/camelcase
...
function isCamelCase(word) {
  return word === camelCase(word);
}

function isPascalCase(word) {
  return word === camelCase(word, { pascalCase: true });
}
...
function isDashCase(word) {
  return dashCaseRegex.test(word);
}
